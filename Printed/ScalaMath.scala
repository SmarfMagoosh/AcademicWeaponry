...
// Matrices
class Matrix(nums: Seq[Seq[Double]]) {
  val determinant: Double = {
    val pos = for i <- (0 until nums.length); l = nums.length yield {
      (for j <- (0 until l) yield nums(j)((j + i) % l)).reduce(_ * _)
    }
    val neg = for i <- (0 until nums.length); l = nums.length yield {
      (for j <- (0 until l) yield nums(j)((l + i - j) % l)).reduce(_ * _)
    }
    pos.sum - neg.sum
  }

  override val toString: String = {
    val rowStrings = for row <- nums; rs = row.toString() yield rs.substring(rs.indexOf("(") + 1, rs.length - 1)
    var ret = ""
    for row <- rowStrings do ret += "|" + row.replace(",", " ") + "|\n"
    ret
  }
}
// Probability
...
def createMatrix(n: Int): Matrix = {
  Matrix(for i <- (0 until n) yield for i <- (0 until n) yield Random.nextInt(5))
}

def time(i: Int): String = {
  val start = System.currentTimeMillis()
  createMatrix(i).determinant
  s"$i x $i: ${(System.currentTimeMillis() - start) / 1000.0}s"
}
...
@main def main(): Unit = {
  for i <- List.range(0, 1000, 100) do println(time(i))
  val test = createMatrix(10)
  println(test)
